...
public class BulkScanGroupHandler {
    ...
    @Autowired private ErrorAndWarningHandler errorAndWarningHandler;

    public Map<MessageTypeEnum, List<String>> handle(
            FormType formType, List<OcrDataField> ocrDataFieldList) {
        Map<MessageTypeEnum, List<String>> errorOrWarning =
                errorAndWarningHandler.getErrorOrWarning();
    ...
    private void traverseFieldTree(
                ...
                .forEach(
                        field -> {
                            if (field instanceof CompositeField) {
                                ...
                                traverseFieldTree(
                                        composite, ocrDataFieldList, errorAndWarningHandler);
                            } else {
                                field.validate(ocrDataFieldList, errorAndWarningHandler);
                            }
                        });
