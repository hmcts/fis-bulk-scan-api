...
import static uk.gov.hmcts.reform.bulkscan.utils.Constants.FGM001_TRANSFORM_REQUEST_PATH;
import static uk.gov.hmcts.reform.bulkscan.utils.Constants.FGM001_TRANSFORM_RESPONSE_PATH;
...
class BulkScanFgm001IntegrationTest {

    @MockBean protected AuthTokenValidator authTokenValidator;

    @Autowired private transient MockMvc mockMvc;

    private static final String FGM001_VALIDATION_REQUEST_PATH =
            "classpath:request/bulk-scan-fgm001-validation-input.json";
    ...
    void shouldTestValidationRequestCaseTypeFgm001() throws Exception {
        ...
        mockMvc.perform(
                        post(FL401_CASE_TYPE_VALIDATE_ENDPOINT)
                                ...
                                .content(readFileFrom(FGM001_VALIDATION_REQUEST_PATH)))
    ...
    void shouldTestTransformRequestCaseTypeFgm001() throws Exception {
        ...
        mockMvc.perform(
                        post(CASE_TYPE_TRANSFORM_ENDPOINT)
                                .contentType(APPLICATION_JSON)
                                .header(SERVICE_AUTHORIZATION, SERVICE_AUTH_TOKEN)
                                .content(readFileFrom(FGM001_TRANSFORM_REQUEST_PATH)))
                .andExpect(status().isOk())
                .andExpect(content().json(readFileFrom(FGM001_TRANSFORM_RESPONSE_PATH)));
    }
}
